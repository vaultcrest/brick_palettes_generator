...
def resolve_studio_part(
    ...
    # Debug area
        ...
        print("Part:", bricklink_part.lower())
        print("Studio Definition unresolved:", definition_match)
    ...
    for item in results:
        ...
        if existing:
            ...
            if same_state and not FORCE_REFRESH and not FORCE_STUDIO_REFRESH:
                ...
                continue
            # Block to update missing data primarily name
            # if existing["bricklink"]["name"] is None:

            #     metadata = fetch_bricklink_item_metadata(
            #         existing["bricklink"]["item_type"],
            #         existing["bricklink"]["part_no"],
            #     )
            #     print( f"Missing name | " f"Element: {existing['lego']['element_id']} | " f"Metadata: {metadata}" )

            #     temp_mapping= lookup_bricklink_mapping(existing['lego']['element_id'])
            #     print( f"temp mapping | " f"{temp_mapping}" )
            #     if metadata.get("name"):

            #         existing["bricklink"]["name"] = metadata["name"]

            #         print(f"Updated missing name for " f"{existing['bricklink']['part_no']} " f"-> {metadata['name']}")
        ...
        #
        # Reject unsupported Studio categories
        #

        reject_reason = should_reject_studio_part(
        ...
        if reject_reason:
            ...
            studio_resolution = {
                ...
                "source_type": None,
                "resolved_from": None,
                "resolution_method": reject_reason,
                "attempted": [],
            }
        ...
        if not studio_part:
            ...
            elif reject_reason == "reject_braille":

                stats["reject_braille"] += 1
